...
import { Search, Filter, MoreVertical, ExternalLink, PlayCircle, Zap } from 'lucide-react';
...
const Incidents: React.FC<IncidentsProps> = ({ role }) => {
  ...
  // Filter incidents for client to only show relevant ones (simulated)
  const displayIncidents = role === UserRole.CLIENT 
    ? incidents.filter(i => i.severity !== 'LOW') // Clients don't see low priority internal tasks
    : incidents;
  ...
  return (
    <div className="space-y-6">
       <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Incident Desk</h1>
